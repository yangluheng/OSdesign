...
//        }
    ...
    public static void main(String[] args) {
        ...
        Scanner in=new Scanner(System.in);
//        testMemory.allocation(10);
//        testMemory.showZones();
//        testMemory.collection(0);
//        testMemory.showZones();
            System.out.println("请输入内存分区大小:");
            int size=in.nextInt();
            testMemory=new TestMemory(size);
            while (true){
                System.out.println("请输入要分配的进程大小:");
                int allocationSize=in.nextInt();
                testMemory.allocation(allocationSize);
                testMemory.showZones();
                System.out.println("是否进行内存回收?y/n");
                String choice=in.next();
                if (choice.equals("y")){
                    System.out.println("请输入要回收的分区编号:");
                    int id=in.nextInt();
                    testMemory.collection(id);
                    testMemory.showZones();
                }
                else {
